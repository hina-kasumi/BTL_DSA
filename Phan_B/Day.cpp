...
private:
    ...
    bool valid(Day day) {
        ...
        if (day.d < 1 || day.d > 31) return false;
        if (day.m == 2) {
...
public:
    Day (int d = 1, int m = 1, int y = 1) {
        ...
        this->m = m;
        this->y = y;
        if (!valid(*this)) throw invalid_argument("Invalid date");
    }

    // các toán tử nhập xuất
    friend istream &operator>>(istream& is, Day &day){
        is >> day.d >> day.m >> day.y;
        if (!day.valid(day)) throw invalid_argument("Invalid date");
        return is;
    }
    ...
    // các toán tử so sánh
    Day operator=(Day day) {d = day.d; m = day.m; y = day.y; return *this;}
